...
pd = pandas.read_csv("/data/06333/aroraish/modifiableN.csv", encoding='utf-8', error_bad_lines=False, chunksize=1000000)

for p in pd:
    p["message"] = p["message"].apply(preprocess)
    p.to_csv(path_or_buf="/data/06333/aroraish/modifiableN_processed.csv", mode='a', index=False)